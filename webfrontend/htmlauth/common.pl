...
sub fetch_window {
  ...
  if (defined $payload && ref($payload) eq 'HASH') {
    ...
    $log->("customerTariffs: returned " . scalar(@$rows) . " rows");
    if (payload_is_complete($payload, $start_iso, $end_iso)) {
  ...
  if ($tariff_name ne '') {
    ...
    if (defined $pub_payload) {
      ...
      $log->("public /tariffs (tariff_name=$tariff_name): returned " . scalar(@$rows) . " rows");
      if (payload_is_complete($pub_payload, $start_iso, $end_iso)) {
  ...
  if (defined $pub_payload && ref($pub_payload) eq 'HASH') {
    ...
    $log->("public /tariffs (no tariff_name): returned " . scalar(@$rows2) . " rows");
    if (payload_is_complete($pub_payload, $start_iso, $end_iso)) {
      eval { publish_mqtt($cfg, $cfg->{mqtt_topic_summary}, { source => 'public', from => $start_iso, to => $end_iso }); 1 };
      return ($pub_payload, 'public');
    }
