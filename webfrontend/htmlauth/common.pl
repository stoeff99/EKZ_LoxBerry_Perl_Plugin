...
sub fetch_window {
    ...
    eval {
        my $payload = get_json_with_retry("$base/customerTariffs", \%hdr,
            { ems_instance_id => $cfg->{ems_instance_id}, start_timestamp => $start_iso, end_timestamp => $end_iso },
            int($cfg->{retries}));
        return ($payload, 'customer');
    } or do {
        my $payload = get_json_with_retry("$base/tariffs", \%hdr,
            { tariff_name => $cfg->{fallback_tariff_name} }, int($cfg->{retries}));
